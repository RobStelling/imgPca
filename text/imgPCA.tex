...
\end{itemize}
If we were to use a rule similar to Kaiser's Rule\footnote{Eigenvalues greater than one}, based on our experience with the proposed tool with $32\times 32$ grayscale images, then we would suggest a $\frac{1}{10}$ Kaiser'r Rule: eigenvalues greater than $0.1$. Trials with a few data sets suggest that eigenvalues $> 0.1$ is a reasonable trade off between compression and sharpness.
\section{Examples}
For the next few examples we used a subset of CIFAR-10 (\citet{krizhevsky2009learning}, chapter 3) images modified to fit our purposes. CIFAR-10 is a collection of $32\times 32$ color images that are available in python, Matlab and binary versions. We converted CIFAR-10 images to grayscale (see \nameref{materials}), rotated the images 90 degrees and produced input files for our tool.
