...
PCA can be thought of as the problem of fitting an n-dimensional ellipsoid to the data, where each axis of the ellipsoid represents a principal component. The larger the axis of a component, the larger the variance for that component.
To find the axis of the ellipsoid we must first centralize, or normalize, the data set, compute the covariance matrix of the data and finally compute the eigenvalues and eigenvectors of the covariance matrix.\par
To achieve that we compute the singular value decomposition of the covariance matrix.
\subsection{Singular Value Decomposition}
If \textit{C} is the covariance matrix of our data set then: \[C = US V^T\] where \textit{S} is a diagonal matrix with positivel real entries and \textit{U} and \textit{V} columns are orthonormal\footnote{both orthogonal and normalized}.\par
Suppose that we have $m$ data points of $n$ dimensions (EG: 2000 images with 1024 pixels, m = 2000, n = 1024) then \[D_{m\times n} \to C_{n\times n}=cov(D) \]
For the singular value decomposition of the covariance matrix we have: \[C_{n\times n} = U_{n\times n}S_{n\times n}V_{n\times n}^T\] where S is a diagonal matrix.
