...
\begin{center}
%\framebox[4.0in]{$\;$}
\includegraphics{CIFAR10All1024RecoveredNormalized.png}
\end{center}
\caption{Sample of CIFAR-10 grayscale images, all classes.}
\label{fig:sampleCifar10}
\end{figure}

For the first example will use $1024$ images with $1024$ ($32\times 32$) pixels from all CIFAR-10 classes, in the order they appear on the original file. Figure \ref{fig:sampleCifar10} displays the same set of images, recovered with $302$ components, based on the "\emph{Tenth Kaiser's Rule}", and the original normalized images. There are a few artifacts on the recovered images but the results seems to be visually satisfactory.\par
If we increase the number of images on the data set, adding additional $1024$ images for a total of $2048$ the 
